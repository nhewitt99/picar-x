...
from picar.interfaces import PWM, ADC, Servo

from picar.utils import reset_mcu

reset_mcu()

from picar.filedb import fileDB
...
class Picarx(object):
    ...
    def __init__(self):
        ...
        self.dir_cal_value = int(
            self.config_flie.get("picarx_dir_servo", default_value=0)
        )
        self.cam_cal_value_1 = int(
            self.config_flie.get("picarx_cam1_servo", default_value=0)
        )
        self.cam_cal_value_2 = int(
            self.config_flie.get("picarx_cam2_servo", default_value=0)
        )
        ...
        self.cali_dir_value = [
            int(i.strip()) for i in self.cali_dir_value.strip("[]").split(",")
        ]
    ...
    @log_on_error(
        logging.ERROR, "Unexpected error when setting motor speed!", reraise=True
    )
    ...
    @log_on_start(
        logging.DEBUG, "Starting to move at speed {speed} with angle {angle}..."
    )
