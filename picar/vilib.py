...
traffic_sign_model_path = (
    "/home/pi/sport_camera/picarx_python_example/tf_150_dr0.2.tflite"
)
gesture_model_path = (
    "/home/pi/sport_camera/picarx_python_example/3bak_ges_200_dr0.2.tflite"
)
...
time_font = lambda x: ImageFont.truetype(
    "/home/pi/sport_camera/picarx_python_example/Roboto-Light-2.ttf", int(x / 320.0 * 6)
)
text_font = lambda x: ImageFont.truetype(
    "/home/pi/sport_camera/picarx_python_example/Roboto-Light-2.ttf",
    int(x / 320.0 * 10),
)
company_font = lambda x: ImageFont.truetype(
    "/home/pi/sport_camera/picarx_python_example/Roboto-Light-2.ttf", int(x / 320.0 * 8)
)
...
def add_text_to_image(name, text_1):
    ...
    # print(rgba_image)
    time_xy = (
        image_target.size[0] - time_size_x - time_size_y,
    ...
    # 设置文本颜色和透明度
    image_draw.text(
        time_xy, time_text, font=time_font(image_target.size[0]), fill=(255, 255, 255)
    )
    image_draw.text(
        company_xy, text_1, font=text_font(image_target.size[0]), fill=(255, 255, 255)
    )
...
class Vilib(object):
    ...
    face_cascade = cv2.CascadeClassifier(
        "/home/pi/sport_camera/picarx_python_example/haarcascade_frontalface_default.xml"
    )
    ...
    detect_obj_parameter["picture_path"] = (
        "/home/pi/picture_file/"
    ...
    def color_detect_object(obj_parameter):
        if obj_parameter == "x":
            ...
            return -1 * (
                int(Vilib.detect_obj_parameter["color_y"] / 160.2) - 1
    ...
    def human_detect_object(obj_parameter):
        if obj_parameter == "x":
            ...
            # print(Vilib.detect_obj_parameter['y'])
            return -1 * (
                int(Vilib.detect_obj_parameter["human_y"] / 160.2) - 1
            )  # max_size_object_coordinate_y
        elif obj_parameter == "width":
    ...
    def gesture_detect_object(obj_parameter):
        if obj_parameter == "x":
            ...
            # print(Vilib.detect_obj_parameter['y'])
            return -1 * (
                int(Vilib.detect_obj_parameter["gesture_y"] / 160.2) - 1
            )  # max_size_object_coordinate_y
        elif obj_parameter == "width":
    ...
    def qrcode_detect_object(obj_parameter="data"):
        if obj_parameter == "x":
            ...
            # print(Vilib.detect_obj_parameter['y'])
            return -1 * (
                int(Vilib.detect_obj_parameter["qr_y"] / 160.2) - 1
            )  # max_size_object_coordinate_y
        elif obj_parameter == "width":
    ...
    def detect_color_name(color_name):
        ...
        Vilib.detect_obj_parameter["upper_color"] = np.array(
            [
                max(Vilib.color_dict[Vilib.detect_obj_parameter["color_default"]]),
                255,
                255,
            ]
        )
            ...
            while True:
                ...
                for frame in camera.capture_continuous(
                    ...
                    if Vilib.detect_obj_parameter["change_setting_flag"] == True:
                        ...
                        change_setting_cmd = (
                            "camera."
                            + Vilib.detect_obj_parameter["change_setting_type"]
                            + "="
                            + str(Vilib.detect_obj_parameter["change_setting_val"])
                        )
                    ...
                    if Vilib.detect_obj_parameter["content_num"] != 0:
                        ...
                        for i in range(Vilib.detect_obj_parameter["content_num"]):
                            ...
                            cv2.putText(
                                ...
                                Vilib.detect_obj_parameter["process_si"][2],
                                2,
                            )
                    ...
                    if Vilib.detect_obj_parameter["setting_flag"] == True:
                        ...
                        if setting_type == "resolution":
                            ...
                            cv2.putText(
                                img,
                                "resolution:"
                                + str(Vilib.detect_obj_parameter["setting_resolution"]),
                                (10, 20),
                                cv2.FONT_HERSHEY_SIMPLEX,
                                0.6,
                                (255, 255, 255),
                            ...
                            cmd_text = (
                                "Vilib.detect_obj_parameter['setting_val'] = camera."
                ...
                Vilib.detect_obj_parameter["picture_path"] = (
                    "/home/pi/picture_file/" + picture_time + ".jpg"
                )
                ...
                a_t = (
                    "sudo raspistill -t 250  -w 2592 -h 1944 -vf"
                    + " -rot "
                ...
                if Vilib.detect_obj_parameter["watermark_flag"] == True:
                    add_text_to_image(
                        Vilib.detect_obj_parameter["picture_path"],
                        Vilib.detect_obj_parameter["watermark"],
                    )
    ...
    def gesture_calibrate(img):
        if Vilib.detect_obj_parameter["calibrate_flag"] == True:
            ...
            # roi_hsv = roi_hsv
            cv2.imwrite(
                "/home/pi/sport_camera/picarx_python_example/cali.jpg",
                img[190:290, 270:370],
            )
    ...
    def traffic_detect(img):
        ...
        if Vilib.detect_obj_parameter["ts_flag"] == True:
            ...
            open_img = cv2.morphologyEx(
                mask_all, cv2.MORPH_OPEN, Vilib.kernel_5, iterations=1
            )  # 开运算
            # open_img = cv2.dilate(open_img, Vilib.kernel_5,iterations=5)

            contours, hierarchy = cv2.findContours(
                open_img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
            ...
            if traffic_n > 0:
                ...
                if traffic_sign_num > 0:
                    ...
                    Vilib.detect_obj_parameter["traffic_sign_t"] = traffic_dict[
                        max_obj_t
                    ]
    ...
    def gesture_recognition(img):
        if Vilib.detect_obj_parameter["gs_flag"] == True:
            ...
            # 首先对样本图像计算2D直方图
            roi_hsv_hist = cv2.calcHist(
                [Vilib.roi_hsv], [0, 1], None, [180, 256], [0, 180, 0, 255]
            )
            # 对得到的样本2D直方图进行归一化
            ...
            if ges_num > 0:
                ...
                if w >= 60 and h >= 60 and face_len == 0:
                    ...
                    if acc_val >= 75:
                        ...
                        cv2.rectangle(
                            img, (0, 0), (125, 27), (204, 209, 72), -1, cv2.LINE_AA
                        )
                        cv2.putText(
                            img,
                            ges_dict[ges_type] + ": " + str(acc_val) + "%",
                            (0, 17),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.6,
                            (255, 255, 255),
    ...
    def human_detect_func(img):
        if Vilib.detect_obj_parameter["hdf_flag"] == True:
            resize_img = cv2.resize(
                img, (320, 240), interpolation=cv2.INTER_LINEAR
            )  # 2.从BGR转换到RAY
    ...
    def color_detect_func(img):
        ...
        if Vilib.detect_obj_parameter["cdf_flag"] == True:
            ...
            mask = cv2.inRange(
                hsv,
                np.array([min(Vilib.color_dict[color_type]), 60, 60]),
                np.array([max(Vilib.color_dict[color_type]), 255, 255]),
            )  # 3.inRange()：介于lower/upper之间的为白色，其余黑色
            if color_type == "red":
            ...
            open_img = cv2.morphologyEx(
                mask, cv2.MORPH_OPEN, Vilib.kernel_5, iterations=1
            )  # 开运算
